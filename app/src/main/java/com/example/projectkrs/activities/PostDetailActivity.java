...
public class PostDetailActivity extends AppCompatActivity {
    ...
    private void savePlaceStatus(String status) {
        ...
        if (currentUser == null) { Toast.makeText(this, "Turite prisijungti", Toast.LENGTH_SHORT).show(); return; }

        String userId = currentUser.getUid();
        FirebaseFirestore db = FirebaseFirestore.getInstance();

        db.collection("users").document(userId)
                .collection("places").document(place.getId())
                .set(new HashMap<String, Object>() {{
                    ...
                    put("lng", place.getLatLng().longitude);
                }})
                .addOnSuccessListener(aVoid -> Toast.makeText(this, "Vieta pažymėta: "+status, Toast.LENGTH_SHORT).show());
    }
}
